import pygame
import math

# Esta clase dibuja la interfaz de usuario (HUD) durante la exploracion del mapa
class GameView:
    def __init__(self, config):
        self.c = config

    def dibujar_interfaz(self, ventana, modelo):
        """
        Dibuja todos los elementos visuales: fondo, carta del personaje y estadisticas.
        """
        jugador = modelo.obtener_jugador_actual()
        if not jugador: return

        # 1. FONDO DINAMICO
        # Cambia la imagen de fondo segun el lugar del mapa donde estes parado
        try:
            ruta_fondo = modelo.obtener_ruta_fondo_nodo()
            fondo = pygame.image.load(ruta_fondo)
            fondo = pygame.transform.scale(fondo, (930, 600))
            ventana.blit(fondo, (0, 0))
        except Exception:
            # Si la imagen no carga, pone un fondo gris para que el texto sea legible
            ventana.fill((40, 40, 40))

        # 2. CARTA PERSONAJE
        # Calcula un efecto de flotacion constante para que el personaje no se vea estatico
        tiempo = pygame.time.get_ticks() / 1000  
        flotacion = math.sin(tiempo) * 8  
        x_carta, y_carta = 100, int(150 + flotacion)
        
        try:
            # Carga la imagen del personaje segun su evolucion actual
            ruta_personaje = modelo.obtener_ruta_imagen_personaje()
            personaje = pygame.image.load(ruta_personaje)
            personaje = pygame.transform.scale(personaje, (230, 320))
            ventana.blit(personaje, (x_carta, y_carta))
        except:
            # Si no hay imagen, dibuja un rectangulo gris como marcador
            pygame.draw.rect(ventana, (100, 100, 100), (x_carta, y_carta, 230, 320))

        # 3. NOMBRE DE EVOLUCION
        # Dibuja el titulo (ej. Gota Joven) justo encima de la carta del personaje
        nombre_evolucion = modelo.info_visual()
        txt_nombre = self.c.f_chica.render(nombre_evolucion, True, self.c.BLANCO)
        ventana.blit(txt_nombre, (x_carta + (230 - txt_nombre.get_width())//2, y_carta - 30))

        # 4. UI DE ESTADISTICAS (Lado derecho de la carta)
        x_info_base = 360 
        
        # VIDAS GLOBALES: Los 3 corazones o intentos que tienes antes del Game Over
        txt_vidas_glob = self.c.f_chica.render("VIDAS:", True, self.c.BLANCO)
        ventana.blit(txt_vidas_glob, (x_info_base, 150))
        
        for i in range(jugador.vidas):
            # Dibuja circulos rojos que representan tus vidas totales
            pygame.draw.circle(ventana, (255, 50, 50), (x_info_base + 110 + (i * 25), 160), 8)

        # PUNTOS DE VIDA (LPs): Tu salud actual para el proximo combate
        fuente_lp = pygame.font.SysFont("Impact", 50)
        # El color cambia a rojo si te queda menos de 30 LP para avisarte del peligro
        color_vida = self.c.NEON if jugador.vida > 30 else (255, 0, 0)
        
        texto_lp = fuente_lp.render(f"{int(jugador.vida)} LP", True, color_vida)
        ventana.blit(texto_lp, (x_info_base, 180))
        
        # UBICACION: Indica en que nodo del grafo te encuentras
        nodo_actual = getattr(jugador, "nodo_actual", "Inicio")
        txt_ubi = self.c.f_chica.render(f"UBICACION: {nodo_actual}", True, self.c.BLANCO)
        ventana.blit(txt_ubi, (x_info_base, 245))
        
        # BARRA DE EXPERIENCIA (XP)
        xp_actual = jugador.xp
        xp_maxima = 100
        ancho_barra_max = 200
        # Calcula cuanto debe llenarse la barra blanca segun tu progreso
        progreso = (xp_actual / xp_maxima) * ancho_barra_max
        
        txt_xp = self.c.f_chica.render(f"EXP: {xp_actual} / {xp_maxima}", True, self.c.BLANCO)
        ventana.blit(txt_xp, (x_info_base, 280))
        
        # Dibuja el fondo negro de la barra y el frente blanco con el progreso real
        pygame.draw.rect(ventana, self.c.NEGRO, (x_info_base, 310, ancho_barra_max, 12)) 
        pygame.draw.rect(ventana, self.c.BLANCO, (x_info_base, 310, progreso, 12))